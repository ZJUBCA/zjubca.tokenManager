...
void membership::transfer(name from, name to, asset quantity, string memo) {
  // lexer
  vector<string> info;
  split(memo, '$', info);
  eosio_assert(info.size() == 3,
               "wrong format! memo requires 3 tokens: action$name$id");
  string opcode = info[0].c_str();
  string kid_name = info[1].c_str();
  string kid_id = info[2].c_str();
  // check actions
  eosio_assert(opcode == "enroll" || opcode == "renew",
               "wrong opcode! currently supporting opcodes: enroll, renew");
  if (opcode == "enroll") {
    // check uniqueness of eos account
    auto new_kid = _members.find(from.value);
    eosio_assert(new_kid == _members.end(),
                 "Member with this account alreadty exists");

    // check uniqueness of student id
    auto id_index = _members.get_index<"studentid"_n>();
    auto new_kid_id = id_index.find(sha256(const_cast<char*>(kid_id.c_str()),
                                           kid_id.size() * sizeof(char)));
    eosio_assert(new_kid_id == id_index.end(),
                 "Member with this id alreadty exists");
    // check format of student id
    eosio_assert(isNum(kid_id), "id is supposed to be a number");
    eosio_assert(kid_id.size() == 10 || kid_id.size() == 8,
                 "wrong digit number");
    if (kid_id.size() == 10) {
      eosio_assert(kid_id[0] == '3' && kid_id[3] == '0' && kid_id[5] == '0',
                   "wrong format for ZJU bachelor id");
    } else if (kid_id.size() == 8) {
      eosio_assert(kid_id[0] == '2' || kid_id[0] == '1',
                   "wrong format for ZJU ms/phd id");
    }
    // check uniqueness of student name
    auto name_index = _members.get_index<"studentname"_n>();
    auto new_kid_name = name_index.find(sha256(
        const_cast<char*>(kid_name.c_str()), kid_name.size() * sizeof(char)));
    eosio_assert(new_kid_name == name_index.end(),
                 "Member with this name alreadty exists");
    // check register fee
